-- =====================================================
-- Complete Medical Scales Database Schema (ADAPTED)
-- =====================================================
-- DeepLuxMed - Medical Scales Repository
-- Version: 2.0.0 (Adapted for user_roles table)
-- HIPAA Compliant with Row Level Security
--
-- ADAPTATIONS:
-- - Removed user_role ENUM creation (uses user_roles table)
-- - Removed profiles table creation (already exists)
-- - Adapted RLS policies to work with user_roles table
-- =====================================================
...
-- Create custom types (excluding user_role ENUM)
...
-- NOTE: profiles table already exists, skipping creation
...
CREATE INDEX IF NOT EXISTS idx_medical_scales_name_trgm ON medical_scales USING GIN (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS idx_medical_scales_description_trgm ON medical_scales USING GIN (description gin_trgm_ops);
...
-- Assessment indexes
CREATE INDEX IF NOT EXISTS idx_assessments_scale_user ON scale_assessments (scale_id, user_id);
CREATE INDEX IF NOT EXISTS idx_assessments_patient ON scale_assessments (patient_id);
CREATE INDEX IF NOT EXISTS idx_assessments_date ON scale_assessments (assessment_date DESC);
CREATE INDEX IF NOT EXISTS idx_assessments_status ON scale_assessments (status);

-- Analytics indexes
CREATE INDEX IF NOT EXISTS idx_usage_metrics_scale_date ON scale_usage_metrics (scale_id, date DESC);
CREATE INDEX IF NOT EXISTS idx_usage_metrics_date ON scale_usage_metrics (date DESC);

-- Security indexes
CREATE INDEX IF NOT EXISTS idx_access_logs_user_accessed ON data_access_logs (user_id, accessed_at DESC);
CREATE INDEX IF NOT EXISTS idx_access_logs_resource ON data_access_logs (resource_type, resource_id);
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_medical_scales_updated_at') THEN
        CREATE TRIGGER tr_medical_scales_updated_at BEFORE UPDATE ON medical_scales 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_scale_questions_updated_at') THEN
        CREATE TRIGGER tr_scale_questions_updated_at BEFORE UPDATE ON scale_questions 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_scale_scoring_updated_at') THEN
        CREATE TRIGGER tr_scale_scoring_updated_at BEFORE UPDATE ON scale_scoring 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_scale_assessments_updated_at') THEN
        CREATE TRIGGER tr_scale_assessments_updated_at BEFORE UPDATE ON scale_assessments 
            FOR EACH ROW EXECUTE FUNCTION update_updated_at();
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_medical_scales_search_vector') THEN
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'tr_scale_assessments_stats') THEN
...
-- NOTE: All other RLS policies will be created by the super_admin migration
-- to avoid type mismatch issues with profiles table
...
GRANT INSERT, UPDATE ON patients, scale_assessments, user_favorites TO authenticated;
