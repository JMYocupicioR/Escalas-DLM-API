...
const botulinumImages_1 = require("@/data/botulinumImages");
...
const generateHtml = (payload) => {
    ...
    const getDosisRange = (musculo, marcaSel) => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const byBrand = dosisDataComplete?.[marcaSel];
    ...
    const puntosMotoresUnicos = Array.from(new Set((musculos || []).map(m => m.nombre)))
        ...
        imagen: (0, botulinumImages_1.getMusculoImage)(nombreMusculo),
        tieneImagen: (0, botulinumImages_1.hasMusculoImage)(nombreMusculo)
    }))
