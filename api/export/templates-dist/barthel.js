...
const BarthelRequestSchema = zod_1.z.object({
    ...
    questions: zod_1.z.array(zod_1.z.object({
        ...
        category: zod_1.z.enum(['autocuidado', 'esfinteres', 'movilidad']),
...
const generateHtml = (payload) => {
    ...
    const getTherapeuticRecommendations = (total) => {
        if (total < 21) {
            return {
    ...
    Object.entries(categorizedQuestions).forEach(([categoryKey, categoryQuestions]) => {
        if (categoryQuestions.length > 0) {
            resultsTable += `<tr class="category-header"><td colspan="3">${categoryNames[categoryKey]}</td></tr>`;
