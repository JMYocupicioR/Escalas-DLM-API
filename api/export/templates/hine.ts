// api/export/templates/hine.ts
import { HineAssessmentData } from '@/hooks/useHineAssessment';
import { hineMotorMilestones, hineBehaviorItems } from '@/data/hine';

export function generateHineReport(data: HineAssessmentData): string {
        ...
        <div class="milestones-section">
            ...
            ${presentMilestones.length > 0 ? `
                ${presentMilestones.map(milestone => {
                  const milestoneData = hineMotorMilestones.find(hm => hm.id === milestone.milestoneId);
                  return `
        ...
        <div class="behavior-section">
            ...
            ${presentBehaviors.length > 0 ? `
                ${presentBehaviors.map(behavior => {
                  const behaviorData = hineBehaviorItems.find(hb => hb.id === behavior.behaviorId);
                  return `
...
function getInterpretationClass(level: string): string {
